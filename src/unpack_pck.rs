...
fn copy_entry_to_output(entry: &mut tar::Entry<GzDecoder<BufReader<File>>>, base_output_path: &Path) -> Result<(), Error> {
    let path = entry.path().context("Failed to get path")?;
    let output_path = base_output_path.to_path_buf().join(path.strip_prefix("/").unwrap_or(&path));
