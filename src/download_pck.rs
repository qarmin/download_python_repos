...
pub fn download_header_check_size(
    client: &Client,
    package: &str,
    url: &str,
    ...
    let content_length = content_length_header
        .to_str()
        .context("Failed to parse content length header to str")?
        .parse::<usize>()
        .context("Failed to parse content length header to usize")?;

    if content_length > MAX_SIZE {
        return Err(anyhow::Error::msg(format!(
            "Ignoring too big source files {package} - {url} - {} MB",
...
pub fn download_packages() {
    ...
    let mut packages_to_check = fs::read_to_string("links.txt")
        .unwrap()
        .split('\n')
        ...
        .collect::<HashSet<_>>();
    let already_downloaded: BTreeSet<String> = fs::read_to_string("already_downloaded_zip.txt")
        .unwrap_or_default()
        .split('\n')
    ...
    let mut already_downloaded = OpenOptions::new()
        .append(true)
        .create(true)
        .open("already_downloaded_zip.txt")
        .unwrap();
